...
function safeSlug(value) {
    return (0, slugify_1.default)(value || "draft", { lower: true, strict: true });
}
...
async function with429Retry(fn, retries = 3) {
    let lastError;
...
Return ONLY a valid JSON object:
{
  ...
  "contentHtml": "Provide the full article HTML here. Ensure the body text alone is at least 1000 words. Include all H2, H3, lists, and FAQ as requested."
    ...
    try {
        const response = await with429Retry(() => ai.models.generateContent({
            model: defaultModel,
        ...
        const contentHtml = aiOutput.contentHtml || "";
        if (!contentHtml.trim()) {
