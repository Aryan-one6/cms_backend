import { extractDocumentMetrics } from "./competitorAnalyzer";
import { averageSentenceLength, countOccurrences } from "./nlp";
import { Benchmarks, ContentScore, ScoreCategory } from "./types";
...
function scoreForRange(value: number, min: number, max: number) {
  if (value === 0 && min === 0 && max === 0) return 100;
...
export function scoreContentAgainstBenchmarks(input: ScoreInput): ContentScore {
  const {
    contentHtml,
    metaTitle = "",
    metaDescription = "",
  ...
  const metrics = extractDocumentMetrics(contentHtml || "<p></p>", baseUrl, trackedTerms);
  ...
  const primaryFrequency =
    metrics.keywordFrequency[keywordLower] || countOccurrences(metrics.rawText.toLowerCase(), keywordLower);

  const secondaryMissing: string[] = [];
  const nlpMissing: string[] = [];
  ...
  secondaryKeywords.forEach((term) => {
    ...
    const freq = metrics.keywordFrequency[lc] || countOccurrences(metrics.rawText.toLowerCase(), lc);
    keywordCounts[term] = freq;
    if (!freq) secondaryMissing.push(term);
  });

  const nlpTargetTerms = (benchmarks.nlpTerms || []).map((t) => t.term.toLowerCase());
  nlpTargetTerms.forEach((term) => {
    const freq = metrics.keywordFrequency[term] || countOccurrences(metrics.rawText.toLowerCase(), term);
    keywordCounts[term] = freq;
    if (!freq) nlpMissing.push(term);
  });
  ...
  const headingScore = (() => {
    ...
    if (!hasH1) actionable.push("Add a single H1 that includes the primary keyword.");
  ...
  const readabilityScore = (() => {
    const sentenceLength = metrics.averageSentenceLength || averageSentenceLength(metrics.rawText);
  ...
  if (lengthScore < 80) actionable.push(`Target ${benchmarks.wordCount.min}-${benchmarks.wordCount.max} words to mirror competitors.`);
  if (keywordPlacementScore < 80) actionable.push("Work the primary keyword into the title, H1, and early H2s.");
  if (nlpCoverageScore < 80 && nlpMissing.length) {
    actionable.push(`Add missing NLP terms: ${nlpMissing.slice(0, 6).join(", ")}`);
  }
  if (mediaScore < 70) actionable.push(`Add ${benchmarks.media.images.min}-${benchmarks.media.images.max} relevant images.`);
